...
API int tethering_enable(tethering_h tethering, tethering_type_e type)
{
	...
	case TETHERING_TYPE_WIFI: {
		...
		g_dbus_proxy_call(proxy, "enable_wifi_tethering",
				g_variant_new("(ssiii)", set.ssid, set.key, set.visibility, set.mac_filter, set.sec_type),
				G_DBUS_CALL_FLAGS_NONE, -1, th->cancellable,
...
API int tethering_wifi_add_allowed_list(tethering_h tethering, const char *mac)
{
	CHECK_FEATURE_SUPPORTED(TETHERING_FEATURE, TETHERING_WIFI_FEATURE);
...
API int tethering_wifi_remove_allowed_list(tethering_h tethering, const char *mac)
{
	_retvm_if(tethering == NULL, TETHERING_ERROR_INVALID_PARAMETER,
			"parameter(tethering) is NULL\n");
	_retvm_if(mac == NULL, TETHERING_ERROR_INVALID_PARAMETER,
...
API int tethering_wifi_add_blocked_list(tethering_h tethering, const char *mac)
{
	_retvm_if(tethering == NULL, TETHERING_ERROR_INVALID_PARAMETER,
			"parameter(tethering) is NULL\n");
	_retvm_if(mac == NULL, TETHERING_ERROR_INVALID_PARAMETER,
...
API int tethering_wifi_remove_blocked_list(tethering_h tethering, const char *mac)
{
	_retvm_if(tethering == NULL, TETHERING_ERROR_INVALID_PARAMETER,
			"parameter(tethering) is NULL\n");
	_retvm_if(mac == NULL, TETHERING_ERROR_INVALID_PARAMETER,
